...
class VendorNormalizer:
    ...
    def __init__(self):
        ...
        self._prefix_patterns = [
            re.compile(p, re.IGNORECASE) for p in self.TRANSACTION_PREFIXES
        ]
        self._suffix_patterns = [
            re.compile(p, re.IGNORECASE) for p in self.SUFFIX_PATTERNS
        ]
    ...
    def detect_transaction_type(
        self, description: str, amount: float, account_type: str = "bank"
    ) -> str:
        """
        ...
        Returns:
            Transaction type: credit, debit, refund, transfer, balance, or ignore
        """
        ...
        for keyword in self.TRANSACTION_TYPE_KEYWORDS["refund"]:
            if self._contains_keyword(desc_lower, keyword):
                return "refund"

        payment_match = any(
            self._contains_keyword(desc_lower, keyword)
            for keyword in self.TRANSACTION_TYPE_KEYWORDS["payment"]
        )
    ...
    def _contains_keyword(description: str, keyword: str) -> bool:
        """Match whole words/phrases so `fee` does not match `coffee`."""
        return re.search(rf"(?<!\w){re.escape(keyword)}(?!\w)", description) is not None
