...
class StatementParser:
    ...
    def __init__(self):
        ...
        self.currency_symbols = {
            '$': 'USD', '€': 'EUR', '£': 'GBP', '¥': 'JPY',
            'C$': 'CAD', 'A$': 'AUD', 'CHF': 'CHF', '₹': 'INR',
            'R$': 'BRL', 'S$': 'SGD', 'HK$': 'HKD', 'NZ$': 'NZD',
            'MXN': 'MXN', 'ZAR': 'ZAR', 'CNY': 'CNY'
        }
        self.currency_codes = ['USD', 'EUR', 'GBP', 'JPY', 'CAD', 'AUD', 'CHF', 
                               'CNY', 'INR', 'MXN', 'BRL', 'ZAR', 'SGD', 'HKD', 'NZD']
    
    def parse_file(self, file_content: bytes, filename: str, account_type: str) -> List[Dict]:
        ...
        try:
            ...
            for col in df.columns:
                ...
                if 'date' in col_lower and date_col is None:
                    date_col = col
                if ('amount' in col_lower or 'transaction' in col_lower) and amount_col is None:
                    amount_col = col
                if ('description' in col_lower or 'merchant' in col_lower or 'details' in col_lower) and desc_col is None:
                    desc_col = col
                if 'currency' in col_lower and currency_col is None:
                    currency_col = col
            
            # Detect currency from first row or filename
            detected_currency = self._detect_currency_from_data(df, amount_col, currency_col)
            ...
            if date_col and amount_col and desc_col:
                for _, row in df.iterrows():
                    try:
                        ...
                        currency = self._extract_currency(amount_str, row.get(currency_col) if currency_col else None)
                        if not currency:
        ...
        for line in lines[1:]:  # Skip header
            ...
            if len(parts) >= 3:
                try:
                    ...
                    amount_str = parts[1]
                    currency = self._extract_currency(amount_str, None) or detected_currency
                    amount = self._extract_amount(amount_str)
    ...
    def _extract_transactions_from_text(self, text: str, account_type: str) -> List[Dict]:
        ...
        for i, line in enumerate(lines):
            ...
            if amount_match and current_date:
                try:
                    ...
                    # Get description (previous or current line)
                    description = lines[i-1] if i > 0 else line
                    description = re.sub(r'[\$]?[\d,]+\.\d{2}', '', description).strip()
                    
                    if description and len(description) > 3:
                        transactions.append({
                            'date': current_date,
                            'amount': abs(amount) if account_type == 'credit_card' else amount,
                            'description': description,
    ...
    def _extract_amount(self, amount_str: str) -> float:
        ...
        is_negative = cleaned.startswith('-') or cleaned.startswith('(') or cleaned.endswith(')')
